// Photography Site - Main JavaScript with Payload CMS Integration
...
const CONFIG = {
  CMS_API_URL: window.location.hostname === 'localhost' 
    ...
    : 'https://cms.benfoggon.com/api', // Payload CMS URL
...
const api = {
  ...
  async getPhotos(params = {}) {
    ...
    if (params.albumId) queryParams.append('where[album][equals]', params.albumId)
    if (params.limit) queryParams.append('limit', params.limit)
    queryParams.append('sort', '-createdAt')
    queryParams.append('depth', '1') // Populate relationships (image, album)
  ...
  async getAlbums(params = {}) {
    ...
    queryParams.append('sort', '-createdAt')
    queryParams.append('depth', '1') // Populate cover image
    
    const query = queryParams.toString()
    const endpoint = `/albums?${query}`
    const response = await this.fetch(endpoint)
    return response.docs || []
  },

  async getAlbum(idOrSlug) {
    // Try to find by slug first, then by ID
    const response = await this.fetch(`/albums?where[slug][equals]=${idOrSlug}&depth=1`)
    if (response.docs && response.docs.length > 0) {
      return response.docs[0]
    }
    // Try by ID
    const byId = await this.fetch(`/albums/${idOrSlug}?depth=1`)
    return byId
  },

  async getAlbumPhotos(albumId) {
    const response = await this.fetch(`/photos?where[album][equals]=${albumId}&depth=1&limit=100`)
    return response.docs || []
  },

  async getSettings() {
    return await this.fetch('/globals/siteSettings')
  },
...
const ui = {
  ...
  renderSettings() {
    ...
    // Update page title
    document.title = settings.siteTitle || 'BF Photography'

    // Update hero section
    const heroTitle = document.querySelector('.hero-title')
    const heroSubtitle = document.querySelector('.hero-subtitle')
    if (heroTitle) heroTitle.textContent = settings.heroTitle || settings.siteTitle || 'BF Photography'
    if (heroSubtitle) heroSubtitle.textContent = settings.heroSubtitle || settings.siteDescription || 'Capturing the Spirit of Motorcycles'

    // Update hero background if set
    if (settings.heroImage) {
      const heroSection = document.querySelector('.hero')
      if (heroSection) {
        const imageUrl = this.getImageUrl(settings.heroImage, 'full')
    ...
    if (aboutContent && settings.aboutText) {
      aboutContent.innerHTML = this.richTextToHTML(settings.aboutText)
    }

    // Update about image if set
    if (settings.aboutImage) {
      const aboutImg = document.querySelector('#about .about-image img')
      if (aboutImg) {
        aboutImg.src = this.getImageUrl(settings.aboutImage, 'card')
      }
    }
  ...
  renderSocialLinks(settings) {
    ...
    // Handle Payload's socialLinks array format
  ...
  renderPhotos(photos = state.photos) {
    ...
    grid.innerHTML = photos.map((photo, index) => {
      ...
      return `
        <div class="gallery-item" data-category="${photo.category || 'other'}" data-aos="fade-up" data-aos-delay="${(index % 3) * 100}">
  ...
  getImageUrl(image, size = 'card') {
    if (!image) return '/assets/images/placeholder.jpg'
    
    // Handle string URL directly
    if (typeof image === 'string') {
      return image.startsWith('http') ? image : `${CONFIG.CMS_API_URL.replace('/api', '')}${image}`
    }
    
    // Payload CMS media format with sizes
    if (image.sizes && image.sizes[size]) {
      const url = image.sizes[size].url
      return url.startsWith('http') ? url : `${CONFIG.CMS_API_URL.replace('/api', '')}${url}`
    }
    ...
    // Filename fallback
    if (image.filename) {
      return `${CONFIG.CMS_API_URL.replace('/api', '')}/media/${image.filename}`
    }
